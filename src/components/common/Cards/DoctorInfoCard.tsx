...
const DoctorInfoCard: React.FC<DoctorProps> = ({ doctor }) => {
  return (
    <div className="flex flex-col w-[238px] h-[375px] rounded-3xl items-center shadow-lg bg-white  hover:shadow-md hover:shadow-drcuotasSecondaryPrimaryColor hover:z-50 hover:scale-105 transition-all duration-300">
      <Image
        src={doctor.user.profile_picture ?? '/images/elements/doctor.svg'}
        alt={doctor.user.first_name}
        className="rounded-t-3xl object-cover bg-red-500"
        width={238}
        height={224}
      />
      <>
