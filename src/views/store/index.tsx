...
export default function StoreView() {
  ...
  useEffect(() => {
    ...
    if (categoryFromUrl && Object.values(SurgeryCategories).includes(categoryFromUrl as SurgeryCategories)) {
      setFilters(prev => ({
        ...prev,
  ...
  const filteredAndSortedSurgeries = useMemo(() => {
    ...
    if (filters.category) {
      result = result.filter((surgery) => surgery.category === filters.category);
    }

    // Filtro por precio
    if (filters.minPrice !== null) {
      result = result.filter((surgery) => (surgery.amount ?? 0) >= filters.minPrice!);
    }
    if (filters.maxPrice !== null) {
      result = result.filter((surgery) => (surgery.amount ?? 0) <= filters.maxPrice!);
    }

    // Filtro por rating
    if (filters.minRating !== null) {
      result = result.filter((surgery) => (surgery.rating ?? 0) >= filters.minRating!);
    }
  ...
  return (
    <div className="w-full min-h-screen p-10">
        <div className="flex flex-col xl:flex-row gap-2">
          {/* Sidebar - Filtros */}
          <div className={`${showFilters ? 'block' : 'hidden'} xl:block xl:w-80 xl:flex-shrink-0`}>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                  ...
                  <span className="text-sm text-gray-600 text-center sm:text-left whitespace-nowrap">
                    {filteredAndSortedSurgeries.length} resultado{filteredAndSortedSurgeries.length !== 1 ? 's' : ''}
                  </span>
                </div>
              </div>
            </div>
                ...
                //  Producto Tienda 
                  ...
                  >
                    ...
                    <div className="w-full sm:w-48 h-48 sm:h-full relative">
                      <img
                        src={surgery.file_banner?.file_link ?? '/images/elements/specialty.svg'}
                    ...
                    <div className="flex-1 p-4 flex flex-col justify-between">
                      <div>
                        ...
                        <span className="text-sm text-gray-500">
                          {categoryLabels[surgery.category as SurgeryCategories] || 'General'}
                        </span>
                      </div>
