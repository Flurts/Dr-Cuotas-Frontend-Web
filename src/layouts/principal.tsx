...
const PrincipalLayout = ({ children, className }: Props) => {
  ...
  return (
    <div className={`${className}`}>
        ...
        <>
          <div className="flex w-20 lg:w-full items-center justify-center gap-2">
            ...
            <>
              <Link
                href="/account"
                className="bg-drcuotasPrimary hover:bg-white text-white hover:text-drcuotasPrimary font-black uppercase leading-tight tracking-tight w-40 h-10 border-2 hover:border-drcuotasPrimary border-drcuotasPrimary  hidden lg:flex flex-row justify-center items-center  group rounded-xl hover:scale-100 transition-all duration-300 gap-2 "
              >
      ...
      {isMenuOpen && (
        <div
          className="fixed inset-0 backdrop-blur-sm bg-black bg-opacity-40 z-50"
          onClick={handleMenuToggle}
        ></div>
      )}
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 right-0 h-full w-64 bg-white shadow-lg transform ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 z-50`}
      >
        <button
        ...
        <div className="flex flex-col items-start mt-12 p-6 space-y-4">
          <Link
            href="/login"
            className="uppercase leading-tight tracking-wide font-black text-lg text-[#737373] hover:text-drcuotasPrimary"
          >
            Login
          </Link>
          <Link
